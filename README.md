...
2.  **自动化流程**：默认按文件名顺序批量处理工作目录下全部 `.iges/.igs` 文件，无需人工干预。
3.  **拓扑修复与法线校正 (BFS)**：
...
4.  **灵活输出**：
    ...
    *   **可视化文件**：支持 `none`, `.vtk`, `.msh`, `.cgns`, `.obj`。
...
### 基本运行
自动按顺序处理目录下所有 `.iges/.igs` 几何文件，使用默认网格尺寸（1.0 ~ 3.0），默认不输出可视化文件（`none`）：
```bash
python main.py
```
...
| 参数         | 默认值     | 说明                                                       |
| :----------- | :--------- | :--------------------------------------------------------- |
| `--input`    | (自动查找) | 指定单个输入几何文件路径（指定后仅处理该文件）              |
| `--size_min` | `1.0`      | 最小网格单元尺寸                                           |
| `--size_max` | `3.0`      | 最大网格单元尺寸                                           |
| `--format`   | `none`     | 输出的可视化格式，支持 `none`, `vtk, msh, cgns, obj` 或 `all` |
...
*   `<几何文件名>.msh`: (默认) Gmsh 原生格式，例如 `c.iges` 对应 `c.msh`。
*   `<几何文件名>.vtk`: 最通用的科学可视化格式。
