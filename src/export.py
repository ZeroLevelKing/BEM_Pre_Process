...
def export_visualization(format_arg, output_dir='out/visual', output_basename='visualization'):
    ...
    export_formats = []
    if format_arg == 'all':
        export_formats = ['vtk', 'msh', 'cgns', 'obj']
    else:
    ...
    if 'vtk' in export_formats:
        gmsh.write(os.path.join(output_dir, f'{output_basename}.vtk'))

    # .msh 是 Gmsh 原生格式，保留信息最全
    if 'msh' in export_formats:
        gmsh.write(os.path.join(output_dir, f'{output_basename}.msh'))

    # .cgns CFD 通用格式，Tecplot 原生支持极佳
    if 'cgns' in export_formats:
        try:
            gmsh.write(os.path.join(output_dir, f'{output_basename}.cgns'))
        except Exception:
            pass  # 忽略不支持 CGNS 的情况

    # .obj 通用 3D 模型格式
    if 'obj' in export_formats:
        gmsh.write(os.path.join(output_dir, f'{output_basename}.obj'))
